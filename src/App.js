...
const testItem = function deleteItem(){
 ...
 */
  ...
  for(let other of itemsList){
    other.addEventListener("click", function showItem(event){
      console.log(`olha ${event.target.innerText}`)
       let deletedList = items.filter(function(item){
        return item.id != event.target.innerText
      })
